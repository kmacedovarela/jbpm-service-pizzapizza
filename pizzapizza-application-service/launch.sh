...
if [ "$MVN_ARG_LINE" != "" ] ; then
    ...
    "$mvnBin" -v
    echo
    projects=( "*-model" "*-kjar" "pizzapizza-application-service")
...
if [[ "$@" =~ "docker" ]]; then
    ...
    docker run -d -p 8090:8090 --name pizzapizza-application-service apps/pizzapizza-application-service:1.0-SNAPSHOT
elif [[ "$@" =~ "openshift" ]]; then
    echo "Launching the application on OpenShift..."
    
    oc new-app pizzapizza-application-service:1.0-SNAPSHOT
    oc expose svc/pizzapizza-application-service
else

	echo "Launching the application locally..."
	pattern="pizzapizza-application-service"
	files=( $pattern )
	cd ${files[0]}
