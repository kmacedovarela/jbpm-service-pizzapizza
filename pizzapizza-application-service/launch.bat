...
if "%*" == "" (
	...
	for %%s in ("-model" "-kjar" "pizzapizza-application-service") do (
...
:startapp
	if not x%mavenInput:docker=%==x%mavenInput% (
		...
		call docker run -d -p 8090:8090 --name pizzapizza-application-service apps/pizzapizza-application-service:1.0-SNAPSHOT
	) else if not x%mavenInput:openshift=%==x%mavenInput% (
		echo Launching the application on OpenShift...
		call oc new-app pizzapizza-application-service:1.0-SNAPSHOT
		call oc expose svc/pizzapizza-application-service
	) else (
		echo "Launching the application locally..."
		cd pizzapizza-application-service
		cd target
